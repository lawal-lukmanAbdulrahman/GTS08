...
export default function CreateProductPage() {
  ...
  const [tags, setTags] = useState("Fashion, Premium");
  const [tagsList, setTagsList] = useState<string[]>(["Fashion", "Premium"]);
  ...
  // No dummy default variant - starts empty
  ...
  // Auto-Save Draft to Cloud Database (debounced 1000ms)
  useEffect(() => {
    const timer = setTimeout(async () => {
      if (name.trim() || basePriceNaira.trim() || variants.length > 0 || imageUrl || description.trim()) {
        ...
        try {
          ...
          const draftPayload = {
            name: name.trim() || "Untitled Draft Product",
            slug: slug.trim() || (name.trim() ? name.toLowerCase().replace(/[^a-z0-9]+/g, "-") : `draft-${Date.now()}`),
            sku: sku.trim() || undefined,
            brand: brand.trim() || "GTS",
          ...
          const res = await fetch(`${API_BASE}/products/drafts`, {
            ...
            body: JSON.stringify({
              id: cloudDraftId,
              title: draftPayload.name || "Untitled New Draft",
              draft_type: "new",
  ...
  const handleNameChange = (val: string) => {
    ...
    if (!sku) {
      const initials = val.replace(/[^a-zA-Z]/g, "").slice(0, 3).toUpperCase() || "GTS";
      setSku(`GTS-${initials}-${Date.now().toString().slice(-4)}`);
    }
  };

  // Profit Margins Engine
  const baseNum = parseFloat(basePriceNaira) || 0;
  const costNum = parseFloat(costPriceNaira) || 0;
  const profitMarginNaira = baseNum > 0 && costNum > 0 ? baseNum - costNum : 0;
  const profitMarginPct = baseNum > 0 && costNum > 0 ? ((profitMarginNaira / baseNum) * 100).toFixed(1) : "0";

  // Upload hero image and automatically bring up the Color Dialog so user can configure color, size & stock
  const handleUploadHeroOrColor = async (file: File) => {
    ...
    try {
      ...
      // Pre-fill Color Dialog with the uploaded image
      ...
      // Open the Color Dialog for the user to specify color name, size, and inventory
      setShowAddColorModal(true);
    } catch (err: any) {
      setErrorMsg("Image upload failed: " + (err.message || "Please check your network connection."));
    } finally {
      setIsUploadingHero(false);
    }
  };

  // Upload color variant image inside the Add Color Dialog
  ...
  const handleConfirmAddColor = () => {
    ...
    const assignedImg = newColorImage || imageUrl || undefined;
    if (!imageUrl && newColorImage) {
      setImageUrl(newColorImage);
      setHasTransparentBg(colorImageStats?.hasTransparentBg ?? false);
    }

    setVariants((prev) => [
      ...prev,
      {
        size: (isCustomSize ? customSizeText : newColorSize).trim() || "Standard",
        color: newColorName.trim(),
        color_hex: "#111827",
        sku: "",
        quantity: Number(newColorQuantity) || 10,
        variant_image_url: assignedImg,
        has_transparent_bg: colorImageStats?.hasTransparentBg ?? (assignedImg?.endsWith(".png")),
      },
    ]);
  ...
  const executeSaveProduct = async (targetStatus: "active" | "draft") => {
    ...
    const parsedTags = tags.split(",").map((t) => t.trim()).filter(Boolean);
    if (brand && !parsedTags.includes(brand)) {
      parsedTags.push(brand);
    }
    if (hasTransparentBg && !parsedTags.includes("transparent-bg")) {
      parsedTags.push("transparent-bg");
    }
    ...
    const payload = {
      name: name.trim() || (targetStatus === "draft" ? "Untitled Draft Product" : ""),
      slug: slug.trim() || (name.trim() ? name.toLowerCase().replace(/[^a-z0-9]+/g, "-") : `draft-${Date.now()}`),
      sku: sku.trim() || undefined,
      brand: brand.trim() || "GTS",
      category_name: categoryName || "General",
    ...
    try {
      ...
      if (targetStatus === "draft") {
        ...
        const res = await fetch(`${API_BASE}/products/drafts`, {
          ...
          body: JSON.stringify({
            id: cloudDraftId,
            title: payload.name || "Untitled New Draft",
            draft_type: "new",
            draft_data: payload,
          }),
        });
        ...
        setSuccessMsg("Product saved to Drafts safely!");
        setTimeout(() => {
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(json.error || `Failed to publish product (${res.status}).`);
        }
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-gray-50 dark:bg-[#121212] text-gray-900 dark:text-white font-sans flex flex-col lg:overflow-hidden">
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-900 dark:text-white truncate max-w-[240px] sm:max-w-[400px]">
                  {name ? name : "Create New Product"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              type="button"
              onClick={handleAttemptBack}
              className="px-3.5 py-1.5 rounded-[6px] border border-gray-200 dark:border-[#333333] hover:bg-gray-100 dark:hover:bg-[#242424] text-xs font-semibold text-gray-700 dark:text-gray-300 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            ...
            <button
              ...
              disabled={saving}
              className="px-3.5 py-1.5 rounded-[6px] border border-amber-400/40 hover:bg-amber-400/10 text-xs font-semibold text-amber-400 dark:text-amber-300 transition-colors cursor-pointer disabled:opacity-50"
            >
              Save as Draft
            </button>
            ...
            <button
              ...
              title={
                !isEligibleToPublish
                  ? "To publish, provide product name, category, price, and at least one color or description image"
                  : "Publish Product Live"
              }
              className={`px-4 py-1.5 rounded-[6px] text-xs font-bold transition-all shadow-md flex items-center gap-2 ${
                !isEligibleToPublish
      ...
      <div className="flex-1 lg:overflow-hidden max-w-[1600px] mx-auto w-full p-4 lg:p-6">
        ...
        {/* Banner Messages */}
        {errorMsg && (
          <div className="mb-5 p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-xs font-semibold flex items-center justify-between">
            <span>⚠️ {errorMsg}</span>
            <button type="button" onClick={() => setErrorMsg("")} className="text-red-400 hover:text-red-600 ml-2">✕</button>
          </div>
        )}
        {successMsg && (
          <div className="mb-5 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold flex items-center justify-between">
            <span>✓ {successMsg}</span>
            <button type="button" onClick={() => setSuccessMsg("")} className="text-emerald-400 hover:text-emerald-600 ml-2">✕</button>
          </div>
        )}

        <form onSubmit={handleValidateAndPublish} className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:h-[calc(100vh-140px)]">
          
          {/* ────── LEFT COLUMN: MAIN SPECS & VARIANTS (2 COLS) ────── */}
          <div className="lg:col-span-2 space-y-6 lg:overflow-y-auto lg:pr-3 [scrollbar-width:thin] [scrollbar-color:rgba(0,0,0,0.15)_transparent] dark:[scrollbar-color:rgba(255,255,255,0.1)_transparent]">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Nexus V8 Pro Smart Washing Machine"
                    className={`w-full px-3.5 py-2.5 rounded-[6px] bg-gray-50 dark:bg-[#202020] border text-sm font-bold text-gray-900 dark:text-white focus:outline-none transition-all ${
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="nexus-v8-pro-smart-washer"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type="text"
                        readOnly
                        value={sku || "Auto-assigned on create"}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      list="subcategory-presets-list"
                      value={subCategory}
                      onChange={(e) => setSubCategory(e.target.value)}
                      placeholder="e.g. Washing Machines, Smartphones"
                    ...
                    />
                    <datalist id="subcategory-presets-list">
                      {(
                        categoriesList.find(
            ...
            <div className="bg-white dark:bg-[#181818] border border-gray-200 dark:border-[#262626] rounded-2xl p-6 shadow-2xs space-y-5">
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-[#262626] pb-3">
                ...
                <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 font-semibold">
                  Gross Margin: {profitMarginPct}% (₦{profitMarginNaira.toLocaleString()})
                </span>
              </div>
            ...
            {/* Card 2: Variants Matrix & Color Groups */}
            <div className="bg-white dark:bg-[#181818] border border-gray-200 dark:border-[#262626] rounded-2xl p-6 shadow-2xs space-y-5">
                ...
                <div className="space-y-4">
                  {Array.from(groupedColorsMap.entries()).map(([_colorName, group], gIdx) => {
                    return (
                      <div
                        key={gIdx}
                        className="p-4 rounded-[12px] bg-gray-50/70 dark:bg-[#202020]/70 border border-gray-200 dark:border-[#2C2C2C] space-y-3.5"
                      >
                        {/* Color Header Bar */}
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div
                  ...
                  onClick={() => {
                    if (!newColorImage) colorFileInputRef.current?.click();
                  }}
                  className={`p-4 rounded-[12px] border-2 border-dashed transition-all flex flex-col items-center justify-center gap-2 text-center min-h-[130px] ${
                    isDraggingColor
                      ? "border-[#EDCF5D] bg-[#EDCF5D]/10"
                      : "border-gray-300 dark:border-[#333333] hover:border-[#EDCF5D] bg-gray-50/50 dark:bg-[#202020]/50"
                  } ${newColorImage ? "p-2" : "cursor-pointer"}`}
                >
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#1C1C1C] border border-red-500/30 dark:border-red-500/40 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
              Since some parts were not completed, this product cannot be published live yet. Would you like to save it as a <strong>Draft</strong> so you can finish it later?
            </p>
          ...
          <div className="bg-white dark:bg-[#1C1C1C] border border-gray-200 dark:border-[#2C2C2C] rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 animate-in fade-in zoom-in-95 duration-150 font-sans">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  Your in-progress product will be saved to cloud drafts so you can finish setting it up later.
                </p>
              </div>
            </div>
